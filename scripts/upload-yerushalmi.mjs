/**
 * Batch upload "Yerushalmi" Torah readings to Supabase + generate Whisper sync.
 *
 * These audios are public_audios (label = "Yerushalmi"), so they are available to
 * EVERY user — including students who do not have a teacher assigned.
 *
 * Usage:
 ...
 *   ONLY_FOLDERS=bereshit \
 *   node scripts/upload-yerushalmi.mjs
 *
 * Options (env vars):
 *   ONLY_FOLDERS=bereshit,noaj   restrict to these folders (comma separated)
...
const ONLY_FOLDERS     = (process.env.ONLY_FOLDERS || '').split(',').map(s => s.trim()).filter(Boolean)
const CONCURRENCY      = parseInt(process.env.CONCURRENCY || '3', 10)
const DRY_RUN          = !!process.env.DRY_RUN
const SKIP_UPLOAD      = !!process.env.SKIP_UPLOAD
const BUCKET           = 'public-audios'
const LABEL            = 'Yerushalmi'
const STORAGE_PREFIX   = 'yerushalmi'
...
function buildJobs() {
  const jobs = []
  let folders = fs.readdirSync(AUDIO_DIR).filter(f =>
    fs.statSync(path.join(AUDIO_DIR, f)).isDirectory())
  if (ONLY_FOLDERS.length) folders = folders.filter(f => ONLY_FOLDERS.includes(f))
  ...
  for (const folder of folders) {
    ...
    for (const file of files) {
      ...
      jobs.push({
        ...
        storagePath: `${STORAGE_PREFIX}/${parashaId}/${aliyahIdx}.m4a`,
      })
    }
  }
  return jobs
...
async function uploadFile(job) {
  if (SKIP_UPLOAD) {
    ...
    const { data } = await supabase.storage.from(BUCKET).list(`${STORAGE_PREFIX}/${job.parashaId}`)
...
async function main() {
  console.log(`\n=== Yerushalmi audio batch upload ===`)
  console.log(`Audio dir:   ${AUDIO_DIR}`)
  console.log(`Only folders:${ONLY_FOLDERS.length ? ' ' + ONLY_FOLDERS.join(', ') : ' (all)'}`)
