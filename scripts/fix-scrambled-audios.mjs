...
function align(whisperWords, sefariaWords) {
  ...
  {const td=whisperWords[whisperWords.length-1]?.end??1,gs=sLen/td,MS=Math.max(4.0,gs*3);let ch=true;while(ch){ch=false;for(let x=1;x<ki.length;x++){const p=ki[x-1],c=ki[x],ts=out[c].start-out[p].start;if(ts<=0||(c-p)/ts>MS){anchorSet.delete(c);out[c]=null;ki.splice(x,1);ch=true;break}}}}
